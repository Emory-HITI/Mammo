...
- **Radiology-modality fusion (MG + US + MRI) — the near-term version of "the patient, not the pixel."** Today almost all deployed AI reads a single modality. Each modality answers a different question: mammography for calcifications and architecture, ultrasound for the dense breast, MRI for extent and enhancement. Fusing them improves lesion characterization: a combined **MG + ultrasound** model reached **AUC ~0.88, ~13–16 points over single-modality**; three-tier frameworks (e.g., TransFusion-BCNet, 2026) fuse MG + US + MRI + clinical; ultrafast DCE-MRI + clinical fusion improves lesion classification. Your group's RSNA26 work asks whether **MG + MRI findings complement pathology-derived features for pCR**. **[your group on the pCR work; VERIFY exact fusion citations — mostly recent algorithmic papers, present as an emerging direction, not trial-level evidence.]**
